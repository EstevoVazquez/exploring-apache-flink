...
public class Aggregation
{
    public static void main(String[] args) throws Exception
    {
        ...
        mapped.keyBy(0).sum(3).writeAsText(params.get("output").concat("/out1"));

        mapped.keyBy(0).min(3).writeAsText(params.get("output").concat("/out2"));

        mapped.keyBy(0).minBy(3).writeAsText(params.get("output").concat("/out3"));

        mapped.keyBy(0).max(3).writeAsText(params.get("output").concat("/out4"));

        mapped.keyBy(0).maxBy(3).writeAsText(params.get("output").concat("/out5"));
        // execute program
